...
def install_system_fonts(*args):
    try:
        if sys.platform == "win32":
            import winreg  # Модуль для работы с реестром Windows
            import ctypes  # Для обновления кэша шрифтов в реальном времени
        ...
        for src in font_files:
            ...
            if not os.path.exists(target):
                ...
                # Для Windows формируем список для записи в реестр
                if sys.platform == "win32":
                    # Имя шрифта в реестре (например, "Aptos Regular (TrueType)")
                    font_name = filename.replace(".ttf", "").replace("-", " ").title() + " (TrueType)"
                    win_reg_keys.append((font_name, filename))

        if changed:
            if sys.platform == "win32" and win_reg_keys:
                # Регистрируем шрифты в пользовательском реестре Windows
                reg_path = r"Software\Microsoft\Windows NT\CurrentVersion\Fonts"
                with winreg.OpenKey(winreg.HKEY_CURRENT_USER, reg_path, 0, winreg.KEY_SET_VALUE) as key:
                    for font_name, win_filename in win_reg_keys:
                        winreg.SetValueEx(key, font_name, 0, winreg.REG_SZ, win_filename)
                
                # Команда Windows API, заставляющая систему и LibreOffice обновить список шрифтов прямо сейчас
...
g_exportedScripts = (install_system_fonts,)
EOF
}
...
<description xmlns="http://openoffice.org/extensions/description/2006" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  <dependencies>
    <l:LibreOffice-minimal-version value="26.2" d:name="LibreOffice 26.2" xmlns:l="http://libreoffice.org/extensions/description/2011" xmlns:d="http://openoffice.org/extensions/description/2006"/>
  </dependencies>
...
<oor:component-data xmlns:oor="http://openoffice.org" xmlns:xs="http://w3.org" oor:name="Jobs" oor:package="org.openoffice.Office">
  <node oor:name="Jobs">
    <node oor:name="FontInstallJob" oor:op="fuse">
      <prop oor:name="Service">
        <value>com.sun.star.task.Job</value>
      </prop>
      <prop oor:name="Macro">
        <value>vnd.sun.star.script:font_installer.py\$install_system_fonts?language=Python&amp;location=user:uno_packages/MS_Office_Aspect_Pack.oxt</value>
      </prop>
    </node>
  </node>
